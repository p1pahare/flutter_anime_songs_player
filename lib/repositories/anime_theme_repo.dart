...
class AnimeThemeRepository extends GetConnect {
  ...
  Future<ApiResponse> searchAnimethemesMain(String title,
    ...
    try {
      final response = await get(isUrl
          ? title
          : '${Values.baseUrl}/animetheme?include=animethemeentries.videos,animethemeentries.videos.audio,anime.images,song.artists&fields[anime]=name,slug,year,season&fields[animetheme]=id,type,sequence,slug&fields[animethemeentry]=id,version&fields[video]=id,tags,link,filename&fields[image]=facet,link&fields[song]=title&fields[artist]=name,slug&filter[has]=song&page[size]=15&page[number]=1&q=${percentEncode(title)}');
//&filter[has]=song
      String body = response.bodyString ?? 'Something Went Wrong';
